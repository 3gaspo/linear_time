...
def load_manifest(path_or_run: str | Path) -> dict[str, Any]:
    ...
    if not {"dataset", "L", "H", "model"} <= set(manifest.get("identity", {})):
        raise ManifestError(f"{path} does not use the current ridge task identity")
...
def allocate_run(
    ...
    *,
    ...
    if os.environ.get("TIME_REUSE_FROM") or os.environ.get("TIME_REUSE_IF_AVAILABLE_FROM"):
        raise ManifestError("Ridge tasks reuse only complete exact tasks in their own identity directory")
    ...
    *,
    ...
    for identity_group in by_identity:
        ...
        differing_keys = {
            ...
            if key != "pipeline_config.seed" and len(
                {
